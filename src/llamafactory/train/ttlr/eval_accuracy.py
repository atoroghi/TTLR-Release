from .eval_utils import extract_gsm8k_answer_number, extract_math_answer, extract_logiqa_option, extract_colota_answer
import json
    ...
    for path in paths:
        ...
        with open(path, 'r', encoding='utf-8') as file:
            for i, line in enumerate(file):
                ...
                if "logiqa" in path:
                    result = extract_logiqa_option(completion)
                elif "gsm8k" in path:
                    result = extract_gsm8k_answer_number(completion)
                elif "meta" in path:
                    result = extract_math_answer(completion, data["label"])
                elif "colota" in path or "CoLoTa" in path:
                    result = extract_colota_answer(completion)
...
if __name__ == "__main__":
    paths = [
        ...
        "updated_predictions_generated_predictions_colota_selected.jsonl"
    ]
    eval_accuracy(paths)
...
class COMEModel(nn.Module):
    def __init__(self, 
        ...
        self.model = enable_norm_only(model)
    ...
    def forward(self, train_batch, predict_batch):
        if self.finetuning_args.setting == "offline_come":
            self.forward_for_offline(train_batch=train_batch, predict_batch=predict_batch)
        
        elif self.finetuning_args.setting == "online_come":
    ...
    def forward_for_offline(self, train_batch, predict_batch):  
        ...
        print('self.training_args.resume_from_checkpoint:', self.training_args.resume_from_checkpoint)
...
def enable_norm_only(model):
    for name, param in model.named_parameters():
        param.requires_grad = False
        if any(k in name.lower() for k in ["rmsnorm", "layernorm", "norm"]):
...
def run_come(
    ...
    if finetuning_args.setting == "offline_come":
        come_model.forward(train_batch=train_dataset, predict_batch=eval_dataset)
    elif finetuning_args.setting == "online_come":
        ...
        for k in range(num_of_batch):
            ...
            come_model.forward(train_batch=sub_trainset, predict_batch=sub_evalset)
    else:
        raise ValueError(

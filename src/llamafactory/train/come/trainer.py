...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        loss, _generated_ids = self.come_generation_entropy_loss(model, input_ids)
        total_loss = loss
    ...
    def dirichlet_entropy(self, logits: torch.Tensor) -> torch.Tensor:
        """
        COME Dirichlet entropy adapted for language models.
        ...
        Returns:
            entropy: (batch_size,)
        """
        # normalize direction, detach magnitude
        ...
        brief = exp_logits / (sum_exp + 1000.0)
        uncertainty = 1000.0 / (sum_exp + 1000.0)

        probability = torch.cat([brief, uncertainty], dim=-1) + 1e-7
        entropy = -(probability * torch.log(probability)).sum(dim=-1)
    ...
    @torch.enable_grad()
    def come_generation_entropy_loss(
        self,
        model,
        ...
        """
        Autoregressively generate tokens and compute COME entropy loss
        with DETACHED transformer backbone (TENT-consistent).
        ...
        Returns:
            loss: scalar tensor
            generated_ids: (batch_size, seq_len + generated_tokens)
        """
        ...
        for _ in range(max_new_tokens):
            ...
            # --------------------------------------------------
            # 3) Forward through lm_head (+ norms if any)
            # --------------------------------------------------
            ...
            # --------------------------------------------------
            # 6) COME entropy loss (Dirichlet entropy)
            # --------------------------------------------------
            entropy_t = self.dirichlet_entropy(logits)  # (B,)

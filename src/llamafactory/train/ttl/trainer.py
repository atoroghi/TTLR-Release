...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def save_predictions(self, dataset: "Dataset", predict_results: "PredictionOutput") -> None:
        ...
        # # Decode original labels from dataset
        # original_labels = np.where(
        #     dataset["labels"] != IGNORE_INDEX, dataset["labels"], self.tokenizer.pad_token_id
        # )
        # decoded_original_labels = self.tokenizer.batch_decode(original_labels, skip_special_tokens=True)

        with open(output_prediction_file, "a", encoding="utf-8") as writer:
            res: List[str] = []
            # for text, original_label, output_label, pred in zip(decoded_inputs, decoded_original_labels, decoded_labels, decoded_preds):
            #     res.append(json.dumps({"prompt": text, "label": original_label, "output label": output_label, "predict": pred}, ensure_ascii=False))

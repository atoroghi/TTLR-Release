...
YES_TOKENS = {
    "yes", "true", "correct", "affirmative", "indeed", "right"
}

NO_TOKENS = {
    "no", "false", "incorrect", "negative", "wrong"
}
ANSWER_PATTERNS = [
    r"the answer is (yes|no|true|false|correct|incorrect)",
    r"therefore[, ]+(yes|no|true|false)",
    r"so[, ]+(yes|no|true|false)",
    r"thus[, ]+(yes|no|true|false)",
    r"final answer[: ]+(yes|no|true|false)",
]
def normalize(token):
    token = token.lower()
...
def extract_by_vote(text):
    tail = text
    tokens = re.findall(r"\b\w+\b", tail)

    yes_count = sum(t.lower() in YES_TOKENS for t in tokens)
    no_count  = sum(t.lower() in NO_TOKENS  for t in tokens)
    ...
    if no_count > yes_count:
        return "no"
    return None
def extract_colota_answer(completion: str):
    """
    This is a "yes" or "no" answer extraction function for the CoLoTa dataset.
    """
    # 1) Explicit answer pattern
    ans = extract_explicit_answer(completion)
    if ans:
        if ans == "yes":
            return "true"
        if ans == "no":
    ...
    # 2) Tail voting
    ans = extract_by_vote(completion)
    if ans:
        if ans == "yes":
            return "true"
        if ans == "no":

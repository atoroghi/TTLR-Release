...
class TENTModel(nn.Module):
    ...
    def forward(self, train_batch, predict_batch):
        if self.finetuning_args.setting == "offline_tent":
            self.forward_for_offline(train_batch=train_batch, predict_batch=predict_batch)
        
        elif self.finetuning_args.setting == "online_tent":
...
def run_tent(
    ...
    if finetuning_args.setting == "offline_tent":
        tent_model.forward(train_batch=train_dataset, predict_batch=eval_dataset)
    elif finetuning_args.setting == "online_tent":
        ...
        for k in range(num_of_batch):
            ...
            tent_model.forward(train_batch=sub_trainset, predict_batch=sub_evalset)
    else:
        raise ValueError(

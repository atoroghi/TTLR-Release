...
class EATAModel(nn.Module):
    ...
    def forward(self, train_batch, predict_batch):
        if self.finetuning_args.setting == "offline_eata":
            self.forward_for_offline(train_batch=train_batch, predict_batch=predict_batch)
        
        elif self.finetuning_args.setting == "online_eata":
...
def run_eata(
    ...
    if finetuning_args.setting == "offline_eata":
        eata_model.forward(train_batch=train_dataset, predict_batch=eval_dataset)
    elif finetuning_args.setting == "online_eata":
        ...
        for k in range(num_of_batch):
            ...
            eata_model.forward(train_batch=sub_trainset, predict_batch=sub_evalset)
    else:
        raise ValueError(

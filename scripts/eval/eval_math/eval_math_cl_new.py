...
def extract_boxed_answer(text: str) -> Optional[str]:
    ...
    Handles nested braces and trailing garbage text.
    
    Strategy:
    - Finds all \boxed{...} patterns in the text
    - Returns the last one (models often give final answer last)
    - Ignores any text after the boxed answer
    ...
    # This pattern handles nested braces (e.g., \boxed{\frac{1}{2}})
    pattern = r'\\boxed\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}'
    matches = re.findall(pattern, text)
    
    if matches:
        # Return the last boxed answer (usually the final answer)
        # This works even if there's trailing garbage after \boxed{}
...
def extract_final_number(text: str) -> Optional[str]:
    ...
    patterns = [
        ...
        r'answer:\s*([0-9\.\-/]+)',
        r'therefore[,\s]+(?:the\s+)?(?:answer\s+is\s*[:\s]*)?([0-9\.\-/]+)',
...
if __name__ == "__main__":
    paths = [
        ...
        "/Users/armin/Downloads/updated_predictions_generated_predictions_math_twocot_ttlr.jsonl"
    ]
    eval_accuracy(paths)